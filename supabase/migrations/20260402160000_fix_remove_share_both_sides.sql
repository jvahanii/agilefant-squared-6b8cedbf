-- Fix remove_tree_share_with_copy:
--  1. Avoid FK violations when copying backlogs/items (insert with NULL parent_id, then update).
--  2. Give BOTH orgs independent copies after separation.
--     Owner org gets copies of the shared org's items; shared org gets copies of the owner's
--     items (as before).  Both orgs end up with a fully self-contained, independent tree.
