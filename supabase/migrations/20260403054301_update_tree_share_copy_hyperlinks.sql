-- Update remove_tree_share_with_copy to also copy hyperlinks for cloned work items
...
BEGIN
  ...
  -- Copy hyperlinks for cloned work items
  INSERT INTO work_item_hyperlinks (work_item_id, url, alt_text, rank, organization_id)
  SELECT wm.new_id, h.url, h.alt_text, h.rank, _org_id
  FROM work_item_hyperlinks h
  JOIN _wi_id_map wm ON wm.old_id = h.work_item_id;
