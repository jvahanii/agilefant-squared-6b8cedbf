-- Fix restore_organization_backup: tree_statuses inserts must use ON CONFLICT (tree_id, key)
-- instead of ON CONFLICT (id), because the trg_seed_tree_statuses trigger fires on every
-- backlog_trees INSERT and seeds default rows with fresh UUIDs. When the snapshot then
-- tries to insert the backed-up statuses for the same (tree_id, key) pairs, the unique
-- constraint "tree_statuses_tree_id_key_key" fires even though the row ids differ.
-- Affects both overwrite mode (trees deleted + re-inserted → trigger fires) and
-- copy mode (brand-new tree inserted → trigger fires before the copy's status insert).
...
BEGIN
  ...
  IF _mode IN ('overwrite','merge') THEN
    ...
    IF _scope_type IN ('all','trees') THEN
      ...
      -- Tree statuses: the trg_seed_tree_statuses trigger fires on every backlog_trees
      -- INSERT and seeds default rows with fresh UUIDs. We therefore use ON CONFLICT
      -- (tree_id, key) so that both newly-seeded defaults and any pre-existing rows
      -- are overwritten with the snapshot values. The id column is intentionally omitted
      -- from the DO UPDATE clause because it is the primary key and cannot be changed.
  ...
  ELSE -- copy mode: new ids everywhere, names suffixed at tree/backlog level
    ...
    BEGIN
      ...
      LOOP
        ...
        -- trg_seed_tree_statuses fires on the backlog_trees INSERT above and seeds
        -- default rows for _new_tree_id. Use ON CONFLICT (tree_id, key) so that the
        -- snapshot values overwrite those defaults rather than failing with a unique
