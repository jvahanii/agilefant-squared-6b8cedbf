...
/** The rating the picker chose, kept only when it is a whole number 1-5. */
function ratingFor(link: ExtractedLink): number | null {
  return link.rating !== undefined && Number.isInteger(link.rating) && link.rating >= 1 && link.rating <= 5
    ? link.rating
    : null;
}
