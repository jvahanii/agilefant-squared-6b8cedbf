...
export default function AppLayout() {
  ...
  // Added safe fallbacks for stack lengths
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (activeData?.type === "workitem" && overData?.type === "backlog") {
      ...
      draggedIds.filter((id) => id !== overData.workItemId).forEach((id) => {
      ...
      draggedIds.forEach((id) => {
        ...
        if (wi && wi.parentId !== overData.parentId) {
          reparentWorkItem(id, overData.parentId as string | null, overData.treeId as string, (overData.backlogIds as string[])[0] ?? "");
        }
      });
      draggedIds.forEach((id) => reorderWorkItemAmongSiblings(id, overData.index as number, overData.treeId as string, overData.backlogIds as string[]));
    } else if (activeData?.type === "backlog-node" && overData?.type === "backlog-reorder") {
      reorderBacklogAmongSiblings(activeData.backlogId as string, overData.index as number, overData.parentId as string | null, overData.treeId as string);
  ...
  }, [moveWorkItemToBacklog, reparentWorkItem, reorderWorkItemAmongSiblings, reorderBacklogAmongSiblings, moveBacklog, reorderBacklogTree, countWithDescendants]);
      ...
      <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="h-screen flex flex-col overflow-hidden bg-background">
          <header className="h-16 border-b flex items-center px-4 gap-3 bg-card shrink-0 shadow-sm z-10">
            <img alt="Agilefant" className="h-10 w-auto" src="/lovable-uploads/0c81b1b5-dc1d-489d-a1c4-51656484d393.png" />
            <h1 className="text-sm font-bold tracking-tight">Agilefant<sup className="text-xs text-primary ml-0.5 font-mono">2.0</sup></h1>
            <OrgSwitcher />
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <button className="p-2 text-muted-foreground hover:text-foreground transition-colors" onClick={() => exportChangeLogAsCsv()}>
              ...
              <div className="flex items-center gap-1 border-l pl-2">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button onClick={() => signOut?.()} className="p-2 text-muted-foreground hover:text-destructive transition-colors ml-1">
          ...
          <main className="flex-1 min-h-0 relative">
            <ResizablePanelGroup direction="horizontal">
              ...
              <ResizablePanel defaultSize={75} minSize