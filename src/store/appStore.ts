...
interface AppState extends DataSnapshot {
  ...
  addWorkItem: (title: string, parentId: string | null, backlogId: string, treeId: string, requestedRank?: number) => void;
...
export const useAppStore = create<AppState>()((set, get) => {
  ...
  return {
    ...
    addWorkItem: (title, parentId, backlogId, treeId, requestedRank) => {
      ...
      // 1. Target Rank: Default to 0 (top) if nothing requested
      const finalRank = requestedRank ?? 0;

      // 2. Optimized Shifting: Only siblings in same tree/backlog
      Object.values(updatedWorkItems).forEach((wi) => {
        const isSameContext = 
          wi.parentId === parentId && 
          wi.backlogAssignments[treeId] === backlogId;
      ...
      // Immediate UI parent link update
