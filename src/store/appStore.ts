...
import {
  ...
  upsertBacklogs,
  deleteBacklogs,
  upsertBacklogTree,import { create } from "zustand";
import { WorkItem, WorkItemStatus, Backlog, BacklogTree } from "@/types/models";
import {
  loadFromSupabase,
  upsertWorkItem,
...
interface AppState extends DataSnapshot {
  ...
  deleteBacklog: (backlogId: string) => Promise<void>;
...
export const useAppStore = create<AppState>()((set, get) => {
  ...
  return {
    ...
    deleteWorkItem: async (workItemId: string) => {
      ...
      try {
        await deleteWorkItems(idsArray).catch(err => console.warn("DB Delete failed", err));

        // KRIITTINEN: Luo täysin uusi objekti jotta referenssit katkeavat
        const remainingWorkItems = Object.fromEntries(
          Object.entries(state.workItems)
            .filter(([id]) => !idsToDelete.has(id))
            .map(([id, item]) => {
              const cleanItem = {
                ...item,
                childrenIds: item.childrenIds.filter(cid => !idsToDelete.has(cid)),
                parentId: (item.parentId && idsToDelete.has(item.parentId)) ? null : item.parentId
              };
              return [id, cleanItem];
            })
        );
        ...
        internalLog({ action: "Hard Purge", entityType: "work_item", details: `Removed ${idsArray.length} items` });
      } catch (err) {
        console.error("Critical failure in deleteWorkItem", err);
      }
    },
    ...
    deleteBacklog: async (backlogId: string) => {
      const state = get();
      const bl = state.backlogs[backlogId];
      if (!bl) return;

      const blIdsToDelete = new Set<string>();
      const collectBlIds = (id: string) => {
        if (blIdsToDelete.has(id)) return;
        blIdsToDelete.add(id);
      ...
      Object.values(state.workItems).forEach((wi) => {
        ...
        Object.entries(newAssignments).forEach(([tId, bId]) => {
          if (blIdsToDelete.has(bId)) delete newAssignments[tId];
        });
      ...
      try {
        ...
        await deleteBacklogs(Array.from(blIdsToDelete));

        wiIdsToDelete.forEach(id => delete updatedItems[id]);

        const remainingBacklogs = Object.fromEntries(
          Object.entries(state.backlogs)
            .filter(([id]) => !blIdsToDelete.has(id))
            .map(([id, b]) => [id, { ...b, childrenIds: b.childrenIds.filter(cid => !blIdsToDelete.has(cid)) }])
        );

        const remainingTrees = Object.fromEntries(
          Object.entries(state.backlogTrees).map(([id, t]) => [
            id,
            { ...t, rootBacklogIds: t.rootBacklogIds.filter(rid => !blIdsToDelete.has(rid)) }
          ])
        );

        set({
          workItems: updatedItems,
          backlogs: remainingBacklogs,
          backlogTrees: remainingTrees,
          undoStack: [...state.undoStack.slice(-(MAX_UNDO - 1)), snapshot(state)],
        });
      } catch (err) {
