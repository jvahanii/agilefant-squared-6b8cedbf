...
describe("Duplicate Rank", () => {
  ...
  it("cleanse re-ranks siblings", () => {
    ...
    data.workItems[p("wi-2")] = makeWi({ id: p("wi-2"), title: "Item 2", ranks: { [p("bl-1")]: 0 } });
    const result = cleanseData(data);
    const ranks = Object.values(result.data.workItems).map((wi) => wi.ranks[p("bl-1")] ?? 0);
    expect(new Set(ranks).size).toBe(ranks.length);
  });

  it("cleanse converges when fixing one group creates a duplicate in another", () => {
    // wi-a: ranks[bl-1]=0 AND ranks[bl-2]=0 (in both backlogs independently)
    // wi-b: ranks[bl-1]=0 (dup in bl-1 group)
    // wi-c: ranks[bl-2]=1 (no dup in bl-2 group initially)
    // Per-backlog ranks: fixing bt-1:bl-1 group (wi-a=0, wi-b=1) does NOT
    // affect bt-2:bl-2 — wi-a keeps ranks[bl-2]=0, wi-c=1 is still unique.
    const data = baseData();
    ...
    // Verify no duplicate ranks in ANY (treeId, backlogId, parentId) group.
    const groups = new Map<string, number[]>();
    Object.values(result.data.workItems).forEach((wi) => {
    ...
    groups.forEach((ranks, key) => {
      expect(new Set(ranks).size, `duplicate ranks in group ${key}`).toBe(ranks.length);
    });
  });
});
